	...
	bean.connectToFirstDevice = function(win, fail) {
		...
		evothings.ble.startScan(
			function(deviceInfo)
			{
				...
				if ((deviceInfo.name == 'RFduino') && !m.deviceInfo)
				{
					console.log('Found RFduino');
	...
	bean.serialWrite = function(data, win, fail) {
		...
		var partWin = function() {
			if(i == buffer.packetCount) {
				...
				console.log(m.deviceHandle)
				console.log(m.serialChar)
				evothings.ble.writeCharacteristic(m.deviceHandle, m.serialChar, packet, partWin, function(data){
					console.log("fail: rfduno:204");
					console.log(data)
					console.log(bean.bytesToHexString(packet))
				});
				i++;
			}
		}
	...
	*/
	bean.startSerialRead = function(dataCallback, fail) {
		if(!m.haveServices) {
			// if we don't have our services yet, fetch them, then call this function again.
			m.getServices(function() {
				bean.startSerialRead(dataCallback, function(){console.log("success rduino:260")});
			}, function(){console.log("fail rduino:261")});
			return;
		}
		...
		// Enable Notification.
		evothings.ble.writeDescriptor(m.deviceHandle, m.serialDesc, new Uint8Array([1,0]), function(){
			console.log("rfduino:278 success")
		}, fail);
		evothings.ble.enableNotification(m.deviceHandle, m.serialChar, function(data) {
			// Handle incoming data.
			console.log("rfduino:280")
			data = new Uint8Array(data);
	...
	bean.bytesToString = function(data, offset, length) {
		offset = offset || 0;
		length = length || data.byteLength;
		var hex = '';
		for (var i = offset; i < (offset + length); i++) {
			hex += (data[i] >> 4).toString(8);
			hex += (data[i] & 0xF).toString(8);
		}
		return hex;
	},
	...
	m.getServices = function(win, fail) {
		...
		evothings.ble.readAllServiceData(m.deviceHandle, function(services)
		{
			...
			for (var si in services)
			{
				...
				/*
				 Reading services... (this may take some time)
				 LOG:   service: 00001800-0000-1000-8000-00805f9b34fb
				 LOG:     characteristic: 00002a00-0000-1000-8000-00805f9b34fb
				 LOG:     characteristic: 00002a01-0000-1000-8000-00805f9b34fb
				 LOG:     characteristic: 00002a04-0000-1000-8000-00805f9b34fb
				 LOG:   service: 00001801-0000-1000-8000-00805f9b34fb
				 LOG:     characteristic: 00002a05-0000-1000-8000-00805f9b34fb
				 LOG:       descriptor: 00002902-0000-1000-8000-00805f9b34fb
				 LOG:   service: 00002220-0000-1000-8000-00805f9b34fb
				 LOG:     characteristic: 00002221-0000-1000-8000-00805f9b34fb
				 LOG:       descriptor: 00002902-0000-1000-8000-00805f9b34fb
				 LOG:     characteristic: 00002222-0000-1000-8000-00805f9b34fb
				 LOG:     characteristic: 00002223-0000-1000-8000-00805f9b34fb
				 LOG: Done disconnecting from device
				 */
				if(service.uuid == '00002220-0000-1000-8000-00805f9b34fb')
				for (var ci in service.characteristics)
				{
					var characteristic = service.characteristics[ci];
					console.log(' Char '+characteristic.uuid);

					if (characteristic.uuid == '00002221-0000-1000-8000-00805f9b34fb')
					{
